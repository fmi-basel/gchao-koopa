...
def test_colocalize_tracks_identical(df_tracks_one, min_frames, distance, length):
    rows, cols = colocalize.__colocalize_tracks(
        df_tracks_one, df_tracks_one, min_frames, distance
    )
    assert len(rows) == len(cols) == length
    if length == 3:
        assert (rows == [0, 1, 2]).all()
...
def test_colocalize_tracks_matching(
    df_tracks_one, df_tracks_two, min_frames, distance, length
):
    rows, cols = colocalize.__colocalize_tracks(
        df_tracks_one, df_tracks_two, min_frames, distance
    )
    assert len(rows) == len(cols) == length


def test_colocalize_dff_identical(df_tracks_one):
    output = colocalize.colocalize_frames(df_tracks_one, df_tracks_one, "test", 1, 1)
    assert all(output["particle_test"] == output["coloc_particle_test"])


def test_colocalize_dff_matching(df_tracks_one, df_tracks_two):
    # Without 0-values
    output = colocalize.colocalize_frames(df_tracks_one, df_tracks_two, "", 1, 5)
    ...
    output = colocalize.colocalize_frames(df_tracks_one, df_tracks_two, "", 1, 1)
...
def test_colocalize_dft_identical(df_tracks_one):
    output = colocalize.colocalize_tracks(df_tracks_one, df_tracks_one, 1, 1)
    assert all(output["particle"] == output["coloc_particle"])


def test_colocalize_dft_matching(df_tracks_one, df_tracks_two):
    output = colocalize.colocalize_tracks(df_tracks_one, df_tracks_two, 1, 1)
    assert output["coloc_particle"].isna().sum() == 6


def test_colocalize_dft_output_present(df_tracks_one, df_tracks_two):
    output = colocalize.colocalize_tracks(df_tracks_one, df_tracks_two, 5, 1)
